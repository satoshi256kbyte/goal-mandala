...
- 方針: この設定を維持する（変更しない）
- 理由: 
  - 過去に何度も変更を試みたが、安定した改善が得られなかった
  - 設定変更による影響範囲が大きく、予測困難
  - 現状の設定で十分な成功率（92.7%）を達成している
- 対策: テスト間干渉は個別のテスト修正で対応する

#### singleFork設定の意味と重要性

⚠️ **重要: singleFork: false が必須要件**

本プロジェクトでは、`singleFork: false` が**変更不可の必須要件**です。この設定は絶対に変更しないでください。

**理由**: 時間がかかっても構わないため、メモリ効率と安定性を最優先します。
...
export default defineConfig({
  test: {
    ...
    poolOptions: {
      forks: {
        singleFork: false, // 各テストファイル後にワーカーを再起動（必須）
...
singleFork: false の効果:
- 各テストファイル後にワーカープロセスを再起動: メモリが完全にクリアされる
- メモリリークの防止: テスト間でメモリが蓄積されない
- 安定性の向上: 15ファイル以上の連続実行でもメモリ不足が発生しない

singleFork: true の問題:
- メモリの蓄積: 単一ワーカープロセスでメモリが蓄積され、8-9ファイル後にメモリ不足で終了
- Worker exited unexpectedly エラー: メモリ不足により予期せぬ終了が発生

実測データ:
- `singleFork: true`: 8/9ファイル成功（メモリ不足で終了）
- `singleFork: false`: 25/30ファイル成功（メモリ問題なし、残りはテスト実装エラー）

トレードオフ:
- 実行時間: やや増加（ワーカー再起動のオーバーヘッド）
- メモリ使用量: 大幅に削減（各テストファイル後にクリア）
- 安定性: 大幅に向上（メモリ不足エラーなし）

結論: 本プロジェクトでは、安定性とメモリ効率を優先し、`singleFork: false` を必須とします。**この設定は絶対に変更しないでください。**
