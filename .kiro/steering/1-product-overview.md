...
目標を立てたはいいものの、目標達成までの道筋が描けない、努力が継続しない人をサポートすることが目的のツールです。  
利用者が自然言語で目標を入力すると、AIが目標を達成するためのマンダラチャートを生成します。  
マンダラチャートは目標に対して8つのサブ目標を設定し、さらに各サブ目標に対して8つのアクションを設定します。  
AIはサブ目標のアクションを日々のタスクに落とし込みます。  
1つのタスクの所要時間は30分程度で、システムは日々利用者にタスクのリマインドを行います。  
...
主にビジネスパーソンや学生で、3〜6ヶ月程度の中長期的な目標を持つ人を想定しています。  
...
| サブ目標 | 中心の目標を達成するために必要な具体的な目標 | 各サブ目標は8つのアクションを持つ |
...
- AIが目標から8つのサブ目標を作った時に1度利用者に確認を促します
- AIが8つのサブ目標からアクションを作った時に1度利用者に確認を促します
- つまり必ず確認が必要なのは2回です。これ以上の確認は手軽さが失われるので行いません。あとは任意のタイミングで編集をするとします。

### AIの精度

- 抽象的な目標から具体的なタスクを落とし込むのに段階を踏むので、各段階で精度調整をしていきます。
- 目標からいきなり具体的な落とし込みをしてしまうと、マンダラチャートが成立しなくなる上、精度を高めるのが難しくなります。
- 各段階がちょうど良い粒度になるようにする部分こそ必要と予想しています。
- 精度向上はAIの学習、プロンプトの向上も行いますが、類似目標があればそれをベースにすることも考えていきます。
- 最終的には言うほどAIは使わなくなるのが理想です。
- どちらかというと未実装部分をAIで補うという考え方が違いです。
...
- この時、最後に「明日もこのまま行く」か「明日は少し気分を変える」を選択します。
  - 「明日もこのまま行く」の場合、実施すれば完了となるアクションのタスクのうち、2/3を同じアクションまたはマンダラチャート的に隣接したアクションからタスクを選び、リマインドし、残り1個はリマインド時期が古い方の10件からランダムで選びます。
  - 「明日は少し気分を変える」の場合、実施すれば完了となるアクションのタスクのうち、全部をリマインド時期が古い方の10件からランダムで選びます。

### RDB を使う理由

- 本プロダクトは目標＞サブ目標＞アクション＞タスクの階層構造に重要な意味があると考えています
- 進捗の計算、類似データの検索、集計など、SQLを駆使したデータ操作が必要になるでしょう
- ゆえに、NoSQLよりも最初からRDBを採用した方がよいと考えています

### 将来の展望

- 取り急ぎ目標管理を題材としますが、要件からタスクへの落とし込み、伝えたいことから文章構成の作成、ニーズからアプローチの仕方の模索などにも転用できるのではと考えています。
...
- AIによるマンダラチャート生成時の詳細な進捗表示や完了通知は不要です
- タスクリストの見直し機能は不要です
...
- 継続が必要なアクションについて、どれぐらい継続すれば継続中とみなすかの変更機能は不要です
  - 継続が必要なアクションは1週間に一度リマインドする設定で固定です
- RDS Proxyの導入
