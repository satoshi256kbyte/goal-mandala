...
// 動的インポートでハンドラーを読み込み
let subGoalsHandler: any;

describe('SubGoals Handler - Update API', () => {
  let app: any;
  ...
  beforeEach(async () => {
    ...
    subGoalsHandler = (await import('./subgoals')).default;

    app = new Hono();
    app.route('/subgoals', subGoalsHandler);
  });

  describe('PUT /subgoals/:subGoalId', () => {
    const validSubGoalData = {
      id: 'subgoal-123',
      goalId: 'goal-123',
      title: '既存のサブ目標',
      description: '既存の説明',
    ...
    describe('正常系テスト（更新成功）', () => {
      it('有効なデータでサブ目標を更新できる', async () => {
        mockFindUnique.mockResolvedValue(validSubGoalData);
        ...
        const res = await app.request('/subgoals/subgoal-123', {
          method: 'PUT',
        ...
        expect(data.title).toBe('新しいサブ目標タイトル');
    ...
    describe('楽観的ロック競合テスト', () => {
      ...
      it('競合エラー時に最新データを返す', async () => {
        ...
        expect(res.status).toBe(409);
        const data = await res.json();
        expect(data.latestData.title).toBe('他のユーザーが更新したサブ目標');
        expect(data.latestData.updatedAt).toBeDefined();
      });
    });

    describe('権限エラーテスト', () => {
      it('他のユーザーのサブ目標を更新しようとした場合は403エラーを返す', async () => {
        mockFindUnique.mockResolvedValue({
          ...validSubGoalData,
          goal: {
      ...
      it('サブ目標が存在しない場合は404エラーを返す', async () => {
