...
export class ReminderStack extends cdk.Stack {
  ...
  constructor(scope: Construct, id: string, props: ReminderStackProps) {
    ...
    this.alertTopic = new sns.Topic(this, 'ReminderAlertTopic', {
      ...
      topicName: `${config.projectName}-${config.environment}-reminder-alerts`,
    });
    ...
    const logGroup = new logs.LogGroup(this, 'ReminderLogGroup', {
      logGroupName: `/aws/lambda/${config.projectName}-${config.environment}-reminder`,
    ...
    this.reminderFunction = new lambda.Function(this, 'ReminderFunction', {
      functionName: `${config.projectName}-${config.environment}-reminder`,
    ...
    this.reminderRule = new events.Rule(this, 'ReminderRule', {
      ruleName: `${config.projectName}-${config.environment}-reminder-schedule`,
      ...
      enabled: config.environment === 'production', // Only enable in production
    });
    ...
    const errorAlarm = new cloudwatch.Alarm(this, 'ReminderErrorAlarm', {
      alarmName: `${config.projectName}-${config.environment}-reminder-errors`,
    ...
    const durationAlarm = new cloudwatch.Alarm(this, 'ReminderDurationAlarm', {
      alarmName: `${config.projectName}-${config.environment}-reminder-duration`,
    ...
    const emailSentMetric = new cloudwatch.Metric({
      namespace: `${config.projectName}/Reminder`,
    ...
    const emailFailureRateAlarm = new cloudwatch.Alarm(this, 'EmailFailureRateAlarm', {
      alarmName: `${config.projectName}-${config.environment}-email-failure-rate`,
    ...
    new cdk.CfnOutput(this, 'ReminderFunctionArn', {
      ...
      exportName: `${config.projectName}-${config.environment}-reminder-function-arn`,
    });

    new cdk.CfnOutput(this, 'ReminderRuleArn', {
      value: this.reminderRule.ruleArn,
      description: 'ARN of the EventBridge Rule',
      exportName: `${config.projectName}-${config.environment}-reminder-rule-arn`,
    });

    new cdk.CfnOutput(this, 'AlertTopicArn', {
      value: this.alertTopic.topicArn,
      description: 'ARN of the SNS Alert Topic',
      exportName: `${config.projectName}-${config.environment}-alert-topic-arn`,
    });
  }
}
